...
exports.updateCartItem = async (req, res) => {
  try {
    ...
    if (!product) {
      return res
        .status(400)
        .send(`Product with ID ${updatedItem.productId} not found`);
    }

    const totalQuantityInCart = getTotalQuantityOfProduct(
      existingCart,
    ...
    if (totalRequestedQuantity > product.stock) {
      return res
        .status(400)
        .send(
          `Total quantity is more than available stock for ${product.name}`
        );
    }
